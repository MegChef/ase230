---
marp: true
---

# Q3. MVC Software Architecture

---

## Question 3.1

What design pattern separates concerns into three distinct layers to avoid spaghetti code?

1. MVP
2. MVC
3. MVVM
4. MVA

---

**Answer:**

- MVC

---

## Question 3.2

In the restaurant analogy for MVC, what does the Kitchen represent? (Hint: data/law material preparation is done in the kitchen)

1. View
2. Controller
3. Model
4. Router

---

**Answer:**

- Model

---

## Question 3.3

Which MVC component is responsible for receiving user requests and coordinating between other components?

1. Model
2. View
3. Controller
4. Router

---

**Answer:**

- Controller

---

## Question 3.4

(T/F) To implement the REST API using Laravel, we need MVC + Routing.

---

**Answer:**

- True

---

## Question 3.5

What is the correct MVC flow sequence?

1. User Request → Model → Controller → View → User
2. User Request → View → Model → Controller → User
3. User Request → Controller → Model → View → User
4. User Request → Controller → View → Model → User

---

**Answer:**

- User Request → Controller → Model → View → User

---

## Question 3.6

These are **benefits** of MVC architecture. Which is wrong?

1. Separation of concerns
2. Reusability
3. Testability
4. Added complexity

---

**Answer:**

- Added complexity

---

## Question 3.7

In Laravel's MVC structure, where are the Models stored?

1. app/Http/Controllers/
2. app/Models/
3. resources/views/
4. routes/Models/

---

**Answer:**

- app/Models/

---

## Question 3.8

These are responsibilities of the Model layer. Which is wrong?

- Handles data and business logic
- Database interactions
- Data validation
- User interface rendering
- Business rules

---

**Answer:**

- User interface rendering

---

## Question 3.9

In Laravel's MVC structure, where are the Views stored?

1. app/Http/Controllers/
2. app/Models/
3. resources/views/
4. routes/Models/

---

**Answer:**

- resources/views/

---

## Question 3.10

Which technology uses MVC architecture?

- PHP
- Laravel
- MySQL
- XAMPP
- WSL2

---

**Answer:**

- Laravel

---

## Question 3.11

Explain why separating code into Model, View, and Controller layers is better than having all code mixed together in one file.

---

**Answer:**

1. Separation of Concerns
Each part has one responsibility
Easier to understand and maintain
Changes in one layer don't affect others

2. Reusability
Different controllers can use models
Views can display different data
Controllers can use different models

3. Testability
Test each layer independently
Mock dependencies easily
Better code quality

---

## Question 3.12

Compare the manual PHP approach (everything in handlers.php) with the MVC approach in terms of code organization and maintainability.

---

**Answer:**

The manual PHP strategy combines the data handling, processing, and output logic all together into a single file such as handlers.php, which can make the code less readable, challenging to update, and not easily reused. 

On the other hand, the MVC strategy separates the application into distinct layers, separating the application into these separate ideas improves organizational knowledge of the code, allows for easier maintenance, and allows certain parts to be updated or reused independently of other parts. 

The three layers being the Model layer which handles the application's data, the View layer which handles application's presentation, and the Controller layer which processes user input and ties the Model and View layers together. 

---

## Summary

We have 12 questions on this page.

1. How many questions did you answer? (12 / 12)
2. What percentage of questions did you answer? (100%)
3. List question numbers that you cannot answer: N/A
4. (Optional) Explain why you could not answer these questions.
